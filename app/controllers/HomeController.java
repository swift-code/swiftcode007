...
//        List<Long> connectionRequestRecievedUserIds = user.connectionRequestReceived.stream().map(x -> x.sender.id)
//                .collect(Collectors.toList());
//
...
//
//        List<JsonNode> requestors = user.connectionRequestReceived.stream()
//                .map(x -> {
...
public class HomeController extends Controller {
    ...
    public Result getProfile(Long userId) {
        ...
        List<JsonNode> requestors = user.connectionRequestReceived.stream().map(x -> {
            User requestor = User.find.byId(x.sender.id);
