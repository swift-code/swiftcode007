...
public class HomeController extends Controller {
    ...
    public Result getProfile(Long userId) {
        ...
        List<Long> connectionRequestRecievedUserIds = user.connectionRequestRecieved.stream().map(x -> x.sender.id)
                .collect(Collectors.toList());
        ...
        List<JsonNode> requestors = user.connections.stream()
                .map(x -> {
                    User requestor = User.find.byId(x.id);
